...
// Временные типы пока не сгенерируется Prisma Client
...
export class BookRepository {
  async findAll(params: {
    ...
    const { page, limit, search, author, category, genre, status, sortBy, sortOrder } = params;

    const skip = (page - 1) * limit;

    // Простой where объект без сложных типов
    const where: any = {};
